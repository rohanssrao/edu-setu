    ...
    fetch(url, {
      ...
      .then((response) => {
        if (response.status) {
          ...
          message.success(
            "Registration Successful! You can login to the system now."
          );
        } else {
      ...
      .then((response) => {
        ...
        if (
          ...
          if (sessionStorage.getItem("type").toLowerCase() === "professor")
            ...
            message.error("Unknown Type! Please contact the application owner");
        }
      })
      .catch((err) => console.log(err));
  };
              ...
              rules={[
                {
                  required: true,
                  message:
                    "Please input password. This will be used for Login.",
                },
                {
