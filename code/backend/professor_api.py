...
def get_all_postings():
    ...
    try:
        ...
        return prepare_response(False,  "Unable to connect to database.")
    try:
        query = '''POSTING_ID, TITLE, PROFESSOR, DESCRIPTION, USERS.EMAIL,USERS.DISPLAY_NAME, LOCATION, PREREQUISITES, CREATED_AT, UPDATED_AT FROM POSTINGS JOIN USERS ON POSTING.PROFESSOR = USERS.USER_ID '''
        curs.execute(query)
        curs.rowfactory = makeDictFactory(curs)
        response = curs.fetchall()
        try:
